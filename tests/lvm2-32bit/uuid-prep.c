...
int lvnum_from_lvid(union lvid *lvid)
{
 ...
 for (i = 0; i < 32; i++) {
  lv_num *= sizeof(_c) - 1;
  if ((c = (__extension__ (__builtin_constant_p (lvid->id[1].uuid[i]) && !__builtin_constant_p (_c) && (lvid->id[1].uuid[i]) == '\0' ? (char *) __rawmemchr (_c, lvid->id[1].uuid[i]) : __builtin_strchr (_c, lvid->id[1].uuid[i])))))
   lv_num += (int) (c - _c);
  if (lv_num < 0)
   lv_num = 0;
 }
...
int lvid_in_restricted_range(union lvid *lvid)
{
 ...
 for (i = 32 - 3; i < 32; i++)
  if (!((*__ctype_b_loc ())[(int) ((lvid->id[1].uuid[i]))] & (unsigned short int) _ISdigit))
   return 0;
...
int id_valid(struct id *id)
{
 ...
 for (i = 0; i < 32; i++)
  if (!_inverse_c[id->uuid[i]]) {
   print_log(3, "uuid/uuid.c", 143 , -1,"UUID contains invalid character");
   return 0;
  }
...
int id_read_format(struct id *id, const char *buffer)
{
 ...
 while (*buffer) {
  ...
  if (out >= 32) {
   print_log(3, "uuid/uuid.c", 195 , -1,"Too many characters to be uuid.");
   return 0;
  }
...
# 203 "uuid/uuid.c"
 ...
 204
# 203 "uuid/uuid.c"
  , -1,"Couldn't read uuid: incorrect number of " "characters.")
                   ;
  return 0;
 }
...
char *id_format_and_copy(struct dm_pool *mem, const struct id *id)
{
 ...
 if (!(repstr = dm_pool_alloc(mem, 40))) {
  print_log(3, "uuid/uuid.c", 216 , -1,"dm_pool_alloc failed");
  return ((void *)0);
 }

 if (!id_write_format(id, repstr, 40))
  do { print_log(7, "uuid/uuid.c", 221 , 0,"<backtrace>"); return ((void *)0); } while (0);
