...
void create_top(struct list_head *top)
{
    ...
    // drop the first two object (without destroying the nested lists)
    char *ptr1 = (char *) top->next;
    top->next->next->prev = top;
    top->next = top->next->next;
    char *ptr2 = (char *) top->next;
    top->next->next->prev = top;
    top->next = top->next->next;
    free(ptr2 - __builtin_offsetof (struct top_list, link));
    free(ptr1 - __builtin_offsetof (struct top_list, link));
}
