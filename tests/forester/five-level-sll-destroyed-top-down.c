...
{
    struct L4 *item = malloc(sizeof *item);
    item->down = NULL;
...
{
    struct L3 *item = malloc(sizeof *item);
    item->down = NULL;
...
{
    struct L0 *item = malloc(sizeof *item);
    item->down = NULL;
 ...
 * @file test-0234.c
 *
 * @brief hierarchically nested SLS up to L5, properly destroyed
 *
 * @attention
