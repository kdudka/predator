...
public:   // methods
	...
	TT(
		const TT&           t) :
		TTBase<T>(t._lhs, t._label, t._rhs),
		lhsCache(t.lhsCache)
	{
	...
	TT(
		const TT&                                t,
		typename TTBase<T>::lhs_cache_type&      lhsCache) :
		TTBase<T>(lhsCache.lookup(t._lhs->first), t._label, t._rhs),
		lhsCache(lhsCache)
	{ }
	...
	TT(
		...
		const std::vector<size_t>&               index,
		typename TTBase<T>::lhs_cache_type&      lhsCache) :
		TTBase<T>(nullptr, t._label, index[t._rhs]),
		lhsCache(lhsCache)
	{
	...
	~TT()
	{
		this->lhsCache.release(this->_lhs);
	}
...
public:   // data types
	...
	{
		bool operator()(
			typename trans_cache_type::value_type*     lhs,
			typename trans_cache_type::value_type*     rhs) const
		{
...
*/
	void buildLhsIndex(Index<const std::vector<size_t>*>& index) const
	{
		for (typename trans_cache_type::value_type* trans : this->transitions)
		{
			index.add(&trans->first._lhs->first);
		}
	}
...
*/
	...
	typename trans_cache_type::value_type* addTransition(
		const std::vector<size_t>&          lhs,
		const T&                            label,
		size_t                              rhs)
	{
	...
	typename trans_cache_type::value_type* addTransition(
		const typename trans_cache_type::value_type*       transition)
	{
...
*/
		...
		bool                     addFinalStates = true)
	{
		...
		{
			lhs.resize(trans->first._lhs->first.size());
			for (size_t j = 0; j < trans->first._lhs->first.size(); ++j)
				lhs[j] = f(trans->first._lhs->first[j]);
			dst.addTransition(lhs, trans->first._label, f(trans->first._rhs));
		}
		return dst;
	}
...
public:
		...
		const TA<T>&                b)
	{
		...
		for (typename trans_cache_type::value_type* trans : b.transitions)
			dst.addTransition(trans);
		...
		bool                     registerFinalState = true) const
	{
		...
		{
			dst.addTransition(trans);
			if (this->isFinalState(trans->first._rhs))
				dst.addTransition(trans->first._lhs->first, trans->first._label, newState);
		}
		...
		bool                                          registerFinalState = true) const
	{
		...
		for (size_t state : this->finalStates)
		{
			...
			assert(j != states.end());
			for (typename trans_set_type::const_iterator k = this->_lookup(state); k != this->transitions.end() && (*k)->first._rhs == state; ++k)
