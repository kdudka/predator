...
//		this->boxMan->adjustLeaves(tmp2, boxRoot);
		...
		for (TA<label_type>::iterator i = src.begin(*j); i != src.end(*j, i); ++i) {
			...
			for (TA<label_type>::iterator j = tmp2.accBegin(); j != tmp2.accEnd(j); ++j) {
				...
				dst.addTransition(lhs2, this->boxMan->lookupLabel(label2), j->rhs());
			}
		}
		}
	}
	...
	bool boxCut(TA<label_type>& dst, TA<label_type>& complement, const TA<label_type>& src, const std::set<const AbstractBox*>& trigger) {
		...
		for (TA<label_type>::Iterator i = src.begin(*k); i != src.end(*k, i); ++i) {
			...
			if (matched == trigger.size()) {
				...
				dst.addTransition(lhs, this->boxMan->lookupLabel(label), *k);
				FAE::reorderBoxes(cLabel, cLhs);
				complement.addTransition(cLhs, this->boxMan->lookupLabel(cLabel), this->nextState());
			} else return false;
		}
		}
		dst.addFinalStates(src.getFinalStates());
		complement.addFinalState(this->nextState());
		src.copyTransitions(dst, TA<label_type>::NonAcceptingF(src));
...
//		TA<label_type>* ta = this->taMan->alloc();
	...
	bool foldBox(size_t root, const Box* box) {
		...
		for (TA<label_type>::Iterator i = tmp.begin(); i != tmp.end(); ++i) {
			...
			FAE::reorderBoxes(label, lhs);
			ta->addTransition(lhs, this->boxMan->lookupLabel(label), state);
		}
...
*/
	...
	TA<label_type>* mergeRoot(TA<label_type>& dst, size_t ref, TA<label_type>& src, std::vector<size_t>& joinStates) {
		...
		size_t refState = _MSB_ADD(this->boxMan->getDataId(Data::createRef(ref)));
		...
		for (std::set<size_t>::const_iterator i = src.getFinalStates().begin(); i != src.getFinalStates().end(); ++i) {
			joinStates.push_back(this->nextState());
			joinStatesMap.insert(std::make_pair(*i, this->freshState()));
		}
		bool hit = false;
...
//				std::cerr << "merging " << *i << '(' << this->roots[*i] << ')' << " into " << root << '(' << this->roots[root] << ')' << std::endl;
				...
				TA<label_type>* ta = this->mergeRoot(*this->roots[root], i->first, *this->roots[i->first], refStates);
				this->updateRoot(this->roots[root], ta);
...
//			normInfo.addRoot(*i);
	...
	void split(std::vector<TA<label_type>*>& dst, const TA<label_type>& src, size_t baseIndex, const std::set<std::pair<size_t, size_t> >& splitPoints) {
		...
		for (std::set<size_t>::const_iterator i = src.getFinalStates().begin(); i != src.getFinalStates().end(); ++i)
			dst[baseIndex]->addFinalState(stateIndex.translateOTF(*i) + this->stateOffset);
...
//		utils::relPrint(std::cerr, rel);
		TA<label_type> ta(this->taMan->getBackend());
		this->roots[root]->collapsed(ta, rel, stateIndex);
		this->updateRoot(this->roots[root], &ta.uselessAndUnreachableFree(*this->taMan->alloc()));
	}
	...
	void enumerateSelectorsAtLeaf(std::set<size_t>& selectors, size_t root, size_t target) const {
		...
		for (TA<label_type>::iterator i = this->roots[root]->begin(); i != this->roots[root]->end(); ++i) {
			if (i->label()->isNode())
				i->label()->iterate(LeafEnumF(*this, *i, target, selectors));
		}
	}
	...
	void isolateAtLeaf(std::vector<FAE*>& dst, size_t root, size_t target, size_t selector) const {
		...
		for (TA<label_type>::iterator i = this->roots[root]->begin(); i != this->roots[root]->end(); ++i) {
			...
			matched = NULL;
			i->label()->iterate(LeafScanF(*this, *i, selector, target, matched));
			if (matched) {
		...
		for (std::vector<std::pair<const TT<label_type>*, const Box*> >::iterator i = v.begin(); i != v.end(); ++i) {
			...
			if (this->roots[root]->isFinalState(i->first->rhs())) {
	...
	void isolateAtRoot(std::vector<FAE*>& dst, size_t root, F f) const {
		assert(root < this->roots.size());
		assert(this->roots[root]);
		for (std::set<size_t>::const_iterator j = this->roots[root]->getFinalStates().begin(); j != this->roots[root]->getFinalStates().end(); ++j) {
		for (TA<label_type>::iterator i = this->roots[root]->begin(*j), end = this->roots[root]->end(*j, i); i != end ; ++i) {
			FAE fae(*this);
			std::set<const Box*> boxes;
...
*/
	...
	void transitionLookup(const TT<label_type>& transition, size_t offset, Data& data) const {
		...
		assert(FAE::isSelector(ni.aBox, offset));
		const Data* tmp;
		if (!this->isData(transition.lhs()[ni.offset], tmp))
			throw std::runtime_error("FAE::transitionLookup(): destination is not a leaf!");
		data = *tmp;
		FAE::displToData(FAE::readSelector(ni.aBox), data);
	}
	...
	void transitionLookup(const TT<label_type>& transition, size_t base, const std::vector<size_t>& offsets, Data& data) const {
		...
		for (std::vector<size_t>::const_iterator i = offsets.begin(); i != offsets.end(); ++i) {
			...
			assert(FAE::isSelector(ni.aBox, *i));
			const Data* tmp;
			if (!this->isData(transition.lhs()[ni.offset], tmp))
				throw std::runtime_error("FAE::transitionLookup(): destination is not a leaf!");
			data.d_struct->push_back(Data::item_info(*i - base, *tmp));
			FAE::displToData(FAE::readSelector(ni.aBox), data.d_struct->back().second);
		}
	}
	...
	void transitionModify(TA<label_type>& dst, const TT<label_type>& transition, size_t offset, const Data& in, Data& out) {
		...
		if (!this->isData(transition.lhs()[ni.offset], tmp))
			throw std::runtime_error("FAE::transitionLookup(): destination is not a leaf!");
		out = *tmp;
		SelData s = FAE::readSelector(ni.aBox);
		FAE::displToData(s, out);
		Data d = in;
		FAE::displToSel(s, d);
		lhs[ni.offset] = this->addData(dst, d);
		label[ni.index] = this->boxMan->getSelector(s);
		FAE::reorderBoxes(label, lhs);
		dst.addTransition(lhs, this->boxMan->lookupLabel(label), state);
	}
	...
	void transitionModify(TA<label_type>& dst, const TT<label_type>& transition, size_t base, const std::vector<std::pair<size_t, Data> >& in, Data& out) {
		...
		for (std::vector<std::pair<size_t, Data>>::const_iterator i = in.begin(); i != in.end(); ++i) {
			...
			assert(FAE::isSelector(ni.aBox, i->first));
			const Data* tmp;
			if (!this->isData(transition.lhs()[ni.offset], tmp))
...
public:
	...
	size_t nodeCreate(const std::vector<std::pair<SelData, Data> >& nodeInfo, const TypeBox* typeInfo = NULL) {
		...
		FAE::reorderBoxes(label, lhs);
		ta->addTransition(lhs, this->boxMan->lookupLabel(label), f);
		this->roots.push_back(ta);
	...
	void nodeLookupMultiple(size_t root, size_t base, const std::vector<size_t>& offsets, Data& data) const {
		assert(root < this->roots.size());
		assert(this->roots[root]);
		this->transitionLookup(this->roots[root]->getAcceptingTransition(), base, offsets, data);
	}	
	...
	void nodeModifyMultiple(size_t root, size_t offset, const Data& in, Data& out) {
		...
		assert(in.isStruct());
		TA<label_type> ta(this->taMan->getBackend());
		this->transitionModify(ta, this->roots[root]->getAcceptingTransition(), offset, *in.d_struct, out);
		this->roots[root]->copyTransitions(ta);
		TA<label_type>* tmp = this->taMan->alloc();
		ta.unreachableFree(*tmp);
		this->updateRoot(this->roots[root], tmp);
