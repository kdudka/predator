...
struct FieldOfObj: public BlockEntity {
    ...
    FieldOfObj(TValId root_, TOffset off_, TObjType clt_):
...
{
    // acquire object ID
    FieldOfObj *objData = new FieldOfObj(root, off, clt);
        ...
        const TSizeOf               winSize)
{
    ...
    // resolve dst region
    Region *rootDataDst;
    this->ents.getEntRW(&rootDataDst, rootValDataDst->obj);
...
void SymHeap::segSetMinLength(TValId root, TMinLen len)
{
    ...
    const TValId peerAt = dlSegPeer(*this, root);
    const TObjId peer = this->objByAddr(peerAt);
    ...
    AbstractObject *peerData = d->absRoots.getEntRW(peer);
